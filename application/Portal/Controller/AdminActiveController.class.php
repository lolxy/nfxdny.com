...
class AdminActiveController extends AdminbaseController {
	...
	function add(){
		$terms = $this->terms_model->where(array('taxonomy'=>'active'))->order(array("listorder"=>"asc"))->select();
	...
	private  function _lists($status=1){
		$term_id=0;
		$termid=$this->terms_model->field('term_id')->where(array("taxonomy"=>"active"))->select();
    	...
    	$terms = $this->terms_model->where(array('taxonomy'=>'active'))->order(array("term_id in ($term_id)"))->getField("term_id,name",true);
	...
	private function _getTree(){
		...
		$result = $this->terms_model->where(array('taxonomy'=>'active'))->order(array("listorder"=>"asc"))->select();
