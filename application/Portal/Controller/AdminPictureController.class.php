...
class AdminPictureController extends AdminbaseController {
	...
	function _initialize() {
		parent::_initialize();
		$this->picture_model = D("Portal/Posts");
	...
	function add_post(){
		if (IS_POST) {
			...
			$result=$this->picture_model->add($article);
			if ($result) {
				//
	...
	public function edit(){
		...
		$post=$this->picture_model->where("id=$id")->find();
	...
	public function edit_post(){
		if (IS_POST) {
			...
			$article['smeta']=json_encode($_POST['smeta']);
			$article['post_content']=htmlspecialchars_decode($article['post_content']);
			$result=$this->picture_model->save($article);
			if ($result!==false) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	private  function _lists($status=1){
		$term_id=0;
		$termid=$this->terms_model->field('term_id')->where(array("taxonomy"=>"picture"))->select();
    	...
    	$terms = $this->terms_model->where(array('taxonomy'=>'picture'))->order(array("term_id in ($term_id)"))->getField("term_id,name",true);
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			$ids=join(",", $ids);
			if ( $this->picture_model->where("id in ($ids)")->save($data)!==false) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			$ids=join(",", $ids);
			if ( $this->picture_model->where("id in ($ids)")->save($data)) {
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
		...
		if(isset($_POST['ids'])){
			...
			if($status!==false){
				foreach ($_POST['ids'] as $post_id){
					...
					if(empty($count)){
						$status=$this->picture_model->where(array("id"=>$post_id))->delete();
					}
				}
			...
			if(isset($_GET['id'])){
				...
				if($status!==false){
					...
					if(empty($count)){
						$status=$this->picture_model->where("id=$id")->delete();
					}
